...
jep_char_buffer *jep_create_char_buffer()
{
...
void jep_destroy_char_buffer(jep_char_buffer *cb)
{
    if (cb == NULL)
        return;
...
int jep_append_char(jep_char_buffer *cb, jep_char c)
{
...
int jep_append_chars(jep_char_buffer *cb, jep_char *c, int n)
{
    int i, result = 0;
...
void jep_remove_char_at(jep_char_buffer *cb, size_t index)
{
...
int jep_clear_char_buffer(jep_char_buffer *cb)
{
    if (cb == NULL || cb->buffer == NULL || cb->size == 0)
        return 0;
